...
class WebSocketRecognitionClient(QObject):
    ...
    def configure(self, host: str, port: str = "5000") -> None:
        ...
        if self.host.startswith("http://"):
            ...
            self.url = base if base.endswith("/ws/sas") else f"{base}/ws/sas"
        elif self.host:
            self.url = f"ws://{self.host}:{self.port}/ws/sas"
        else:
            self.url = ""
    ...
    def send_ping(self) -> None:
        if self._connected:
            self.send_json({"type": "ping", "client_time": time.strftime("%Y-%m-%d %H:%M:%S")})
