...
class SecureAccessDashboard(QMainWindow):
    ...
    def write_sas_log(self, action: str, actor: str | None = None, details=None):
        """Write a clean plain-text SAS audit record locally and to the server path.

        New logs are stored as ``SAS_LOG/YYYY-MM-DD.txt``. Each entry uses a
        single readable pipe-delimited row: time, actor, action, and details.
        Passwords and other secret values are never written. Existing
        pipe-delimited callers are supported and converted automatically, so
        legacy login/unlock/admin calls remain compatible.
        """
        ...
        try:
            ...
            # 2) Server SAS_LOG under the configured server path. Refresh from
            # credential.txt as a fallback because Save Connect may update it.
            try:
    ...
    def get_settings_ui_snapshot_for_save_prompt(self, fill_blank_sensitive: bool = True):
        ...
        try:
            h = max(0, min(int(getattr(self, "timeout_inputs", {}).get("hours").text() or 0), 23))
            m = max(0, min(int(getattr(self, "timeout_inputs", {}).get("minutes").text() or 0), 59))
            s = max(0, min(int(getattr(self, "timeout_inputs", {}).get("seconds").text() or 0), 59))
            timeout = str(max(1, h * 3600 + m * 60 + s))
        except Exception:
            timeout = str(getattr(self, "lock_timeout_seconds", DEFAULT_LOCK_TIMEOUT_SECONDS))

        try:
            disable_keyboard = bool(getattr(self, "security_option_buttons", {}).get("disable_keyboard").isChecked())
        except Exception:
            disable_keyboard = bool(getattr(self, "disable_keyboard_when_locked", False))
        try:
            disable_mouse = bool(getattr(self, "security_option_buttons", {}).get("disable_mouse").isChecked())
        except Exception:
        ...
        try:
            auto_capture = bool(getattr(self, "auto_capture_toggle").is_enabled())
        except Exception:
            auto_capture = bool(getattr(self, "auto_capture_enabled", False))
        ...
        def worker():
            ...
            def done():
                ...
                settings_audit_details.update({
                    ...
                    "Auto capture": "Enabled" if auto_capture else "Disabled",
            ...
            try:
                    ...
                    def complete():
                        ...
                        if bool(getattr(self, "_training_waiting_for_capture_stop", False)):
                            ...
                            self.write_sas_log(
                                ...
                                actor=actor,
                                details={"Face ID": user_id.upper(), "Captured images": frames, "Mode": "Auto"},
                            )
                ...
                def fail(err=err):
                    ...
                    self.write_sas_log(
                        "FACE CAPTURE FAILED",
                        actor=actor,
                        details={"Face ID": user_id.upper(), "Mode": "Auto" if initial_auto_capture else "Manual", "Result": "Failed"},
                    )
                    self.set_face_capture_in_progress(False)
                    self.set_capture_button_manual_mode(False)
                    self.manual_capture_user = None
                ...
                def done():
                    ...
                    self.write_sas_log(
                        ...
                        details={
                            ...
                            "Encodings removed": encodings_removed,
                        },
                    )
