...
Only user/encoding backups created before a deletion are removed here.  Import
...
def _is_delete_backup_name(name: str) -> bool:
    """Return True only for pre-delete user/encoding safety backups."""
    lower = str(name or "").lower()
    return (
        lower.startswith("deleted_user_backup_")
        or lower.startswith("encodings_backup_before_delete_")
        or lower.startswith("encodings_backup_before_api_delete_")
        or lower == "encodings.pickle.bak"
    )
