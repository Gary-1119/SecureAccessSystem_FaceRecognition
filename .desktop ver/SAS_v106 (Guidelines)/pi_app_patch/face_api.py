...
API_HOST = "0.0.0.0"
API_PORT = 5000
VIDEO_FEED_MAX_FRAME_AGE_SECONDS = 2.0
...
def create_api_app(gui_app):
    ...
    def api_status():
        ...
        latest_frame_at = float(getattr(gui_app, "_api_latest_frame_at", 0.0) or 0.0)
        latest_frame_age = max(0.0, time.monotonic() - latest_frame_at) if latest_frame_at > 0 else None
        ...
        return ok(
            ...
            users=len(users),
            video_frame_age_seconds=latest_frame_age,
            video_frame_fresh=bool(
                latest_frame_age is not None
                and latest_frame_age <= VIDEO_FEED_MAX_FRAME_AGE_SECONDS
            ),
                    ...
                    try:
                        with gui_app._api_frame_lock:
                            if gui_app._api_latest_frame is not None:
                                frame_at = float(getattr(gui_app, "_api_latest_frame_at", 0.0) or 0.0)
                                if (
                                    frame_at > 0
                                    and time.monotonic() - frame_at <= VIDEO_FEED_MAX_FRAME_AGE_SECONDS
                                ):
