...
from websocket_service import attach_websocket_routes, recognition_ws_hub
...
def create_api_app(gui_app):
    ...
    CORS(api_app)
    websocket_enabled = attach_websocket_routes(api_app, gui_app)
    ...
    def api_status():
        ...
        return ok(
            ...
            websocket_available=bool(websocket_enabled),
            websocket_clients=recognition_ws_hub.active_client_count(),
            websocket_lock_sessions=recognition_ws_hub.active_lock_session_count(),
            websocket_last_event_at=recognition_ws_hub.last_event_at,
            websocket_last_error=recognition_ws_hub.last_error,
            sas_oneconnect_session=recognition_ws_hub.active_session_snapshot(),
        )

    # --------------------------------------------------------
    # SAS ONE-PI CONNECTION OWNERSHIP
    # --------------------------------------------------------
    @api_app.route("/sas/session", methods=["GET"])
    def api_sas_session():
        session = recognition_ws_hub.active_session_snapshot()
        return ok(
            "SAS connection session loaded." if session else "No active SAS connection.",
            connected=bool(session),
            session=session,
        )

    @api_app.route("/sas/connect", methods=["POST"])
    def api_sas_connect():
        payload = request.get_json(silent=True) or {}
        client_id = str(payload.get("client_id") or "").strip()
        client_name = str(payload.get("client_name") or "Windows SAS").strip()
        client_user = str(payload.get("client_user") or "").strip()
        client_host = str(payload.get("client_host") or request.remote_addr or "").strip()
        force = bool(payload.get("force"))

        ok_state, message, session = recognition_ws_hub.connect_sas(
            client_id=client_id,
        ...
        if ok_state:
            return ok(message, connected=True, session=session)
    ...
    @api_app.route("/sas/disconnect", methods=["POST"])
    def api_sas_disconnect():
        payload = request.get_json(silent=True) or {}
        client_id = str(payload.get("client_id") or "").strip()
        force = bool(payload.get("force"))

        ok_state, message, session = recognition_ws_hub.disconnect_sas(client_id=client_id, force=force)
        if ok_state:
            return ok(message, connected=False, session=session)
        return fail(message, status=409, reason="not_owner", connected=True, session=session)

    @api_app.route("/sas/heartbeat", methods=["POST"])
    def api_sas_heartbeat():
        payload = request.get_json(silent=True) or {}
        client_id = str(payload.get("client_id") or "").strip()
        ok_state, message, session = recognition_ws_hub.heartbeat_sas(client_id)
        if ok_state:
            return ok(message, connected=True, session=session)
        return fail(message, status=409, reason="not_connected", connected=bool(session), session=session)
    ...
    def api_stop_recognition():
        ...
        Pi stop cancels automatic recognition recovery; SAS registration needs
                ...
                while True:
                    ...
                    try:
                        with gui_app._api_frame_lock:
                            preview_frame = getattr(gui_app, "_api_preview_frame", None)
                            if preview_frame is not None:
                                frame = preview_frame.copy()
                            elif gui_app._api_latest_frame is not None:
    ...
    def api_capture_user():
        ...
        user_id = str(payload.get("user_id") or payload.get("employee_id") or payload.get("ntid") or "").strip()
        display_name = str(payload.get("display_name") or payload.get("name") or payload.get("user_name") or user_id).strip()
    ...
    @api_app.route("/logs/export", methods=["GET"])
    def api_export_logs():
        try:
            if not has_method("export_logs_archive"):
                return fail("Log export service not implemented.", status=501)

            ok_state, message, extra = gui_app.export_logs_archive()
            if not ok_state:
                return fail(message, status=500)

            zip_path = str(extra.get("zip_path") or "")
            filename = str(extra.get("filename") or os.path.basename(zip_path) or "Pi_LOG.zip")
            if not zip_path or not os.path.exists(zip_path):
                return fail("Pi log archive was not created.", status=500)
            ...
            return fail(f"Failed to export logs: {e}", status=500)
