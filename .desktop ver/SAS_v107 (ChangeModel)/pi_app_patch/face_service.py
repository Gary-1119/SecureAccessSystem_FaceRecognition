...
class FaceService:
    ...
    def log(self, message: str, level: str = "info") -> None:
        ...
        if self.gui is not None and hasattr(self.gui, "log"):
            try:
                root = getattr(self.gui, "root", None) or self.root
                if root is not None and threading.current_thread() is not threading.main_thread():
                    root.after(0, lambda message=message, level=level: self.gui.log(message, level))
                else:
    ...
    def _set_gui_frame(self, frame, store_latest: bool = True) -> None:
        with self._api_frame_lock:
            self._api_preview_frame = frame.copy()
            if store_latest:
    ...
    def _load_pc_path_only(self) -> None:
        ...
        try:
            if self.gui is not None:
                ...
                self.gui.auto_capture_enabled = False
        except Exception:
            pass
    ...
    def _api_save_settings_request(self, payload: dict) -> tuple[bool, str, dict]:
        ...
        1. Full server-settings sync (NTID/password/server path).
        2. Rotation-only updates, which do not remount SMB.
        ...
        self.auto_capture_enabled = False
        current_settings["auto_capture"] = False
    ...
    def _api_get_users_list(self) -> list[dict]:
        try:
            ...
            self.log(f"Failed to load InsightFace users: {e}", "error")
            return []
    ...
    # --------------------------------------------------------
    # Recognition / registration / delete
    # --------------------------------------------------------
    def _recognition_frame_callback(self, frame, boxes=None, raw_frame=None):
        with self._api_frame_lock:
            self._api_latest_frame = (raw_frame if raw_frame is not None else frame).copy()
        display = frame.copy()
        if boxes:
            for box in boxes:
                try:
                    ...
                    pass
        self._set_gui_frame(display, store_latest=False)

    def _latest_recognition_frame(self):
        with self._api_frame_lock:
            if self._api_latest_frame is None:
                return None
            return self._api_latest_frame.copy()

    def _register_from_latest_recognition_frame(self, user_id: str, display_name: str, mode: str = "add") -> tuple[bool, str, dict]:
        frame = self._latest_recognition_frame()
        if frame is None:
            return False, "Recognition is running, but no camera frame is ready yet.", {}
        try:
            with self._face_data_lock:
                if mode == "replace":
                    try:
                        get_engine().delete_user(user_id)
                    except Exception:
                        pass
                user = get_engine().register_frame(display_name, user_id, frame)
            self._refresh_dataset_ui()
            self.log(
                f"Registered {display_name} ({user_id}) from live recognition frame. "
                f"Total samples {user.sample_count}.",
                "success",
            )
            return True, "Registration complete from live recognition frame.", {
                "user_id": user_id,
                "employee_id": user_id,
                "name": display_name,
                "display_name": display_name,
                "captured": 1,
                "photos": int(user.sample_count),
                "frames": int(user.sample_count),
                "training_required": False,
                "recognition_interrupted": False,
            }
        except Exception as exc:
            return False, f"Live registration failed: {exc}", {}
    ...
    def _queue_recognition_resume(self, workflow: str, delay_seconds: float = 1.0) -> None:
        ...
        def worker():
            ...
            try:
                ...
                with self._service_lock:
                    self._recognition_resume_pending = False
                    if workflow in ("capture", "registration"):
                        self._resume_recognition_after_capture = False
    ...
    def _capture_frame_callback(self, frame):
        ...
        # Registration stores one embedding only when exactly one face is detected.
    ...
    def _api_manual_capture_request(self):
        ...
        self.log("Manual capture-photo request ignored; direct registration captures automatically.", "info")

        return True, "Direct registration is automatic; no second capture step is required.", {
            "user_id": self._active_capture_user,
            "manual_capture_count": self._manual_capture_count + 1
        }

    def _api_capture_user_request(self, user_id, mode="add", auto_capture=None, display_name=None):
        """Register one InsightFace sample, reusing live recognition frames when possible."""
        ...
        with self._service_lock:
            if self._api_training_running:
                return False, "Training is running. Please wait until it is completed.", {}
            if self._capture_running or self._is_gui_capture_running():
                return False, "Capture is already running.", {}
            recognition_running = bool(self._recognition_running or self._is_gui_recognition_running())
            if recognition_running:
                return self._register_from_latest_recognition_frame(user_id, display_name, mode)
            self._resume_recognition_after_capture = True
            self._active_capture_auto = False
            self._manual_capture_count = 0
            self._active_capture_user = user_id
            self._active_capture_display_name = display_name
        ...
        self._notify_gui_state("capture", f"Registering {display_name} ({user_id})...")
        ...
        def run_capture():
            try:
                ...
                self.log(
                    f"Registration complete for {display_name} ({user_id}). "
                    f"Captured {captured}; total samples {total}.",
                    "success",
                )
                self._refresh_dataset_ui()
            except Exception as e:
                ...
                self._notify_gui_state("error", f"Capture failed: {e}")
            finally:
                with self._state_lock:
                    self._capture_running = False
                    self._stop_capture_event.clear()
                    self._manual_capture_event.clear()
                    self._active_capture_user = None
                    self._active_capture_display_name = None
                    self._active_capture_auto = False
    ...
    def _api_train_request(self) -> tuple[bool, str, dict]:
        """Compatibility no-op: InsightFace registration saves embeddings immediately."""
        training_id = f"train-{int(time.time() * 1000)}"
        self._api_training_running = False
        self._api_training_state = "completed"
        ...
        self._api_training_percent = 100
        self._api_training_phase = "completed"
        self._api_training_message = "No training required. Embedding is saved during registration."
        self.log("REGISTRATION_MODEL_READY: InsightFace embeddings are saved during registration.", "info")
        return True, "No training required. InsightFace embedding is already saved.", {
            "training_id": training_id,
            "training_state": "completed",
            "training_required": False,
            "last_training_new_faces": 0,
        }

    def _api_delete_user_request(self, user_id: str, expected_name: Optional[str] = None) -> tuple[bool, str, dict]:
        user_id = str(user_id).strip().upper()
        expected_name = str(expected_name or "").strip()
        if not user_id:
            return False, "Missing user_id.", {}
        user = get_engine().users.get(user_id)
        if user is None:
            get_engine().reload_if_changed()
            user = get_engine().users.get(user_id)
        if user is None:
            return False, f"No registered user was found for NTID '{user_id}'.", {}
        registered_name = str(getattr(user, "display_name", "") or "").strip()
        if expected_name and registered_name and expected_name.casefold() != registered_name.casefold():
            return False, f"Name mismatch for NTID '{user_id}'. Registered name is '{registered_name}'.", {}
        with self._service_lock:
            if self._capture_running or self._api_training_running:
                return False, "Capture or training is running. Please stop it first.", {}
            should_resume_recognition = bool(self._recognition_running or self._is_gui_recognition_running())

        if should_resume_recognition:
            stopped, stop_message = self._stop_recognition_for_transition("delete")
            if not stopped:
                return False, stop_message, {"recognition_resume_requested": False}

        try:
            with self._face_data_lock:
                result = get_engine().delete_user(user_id)
            self._refresh_dataset_ui()
            self.log(f"User deleted: {user_id}", "warn")
            result["deleted_user_backup_created"] = False
            result["recognition_was_running"] = should_resume_recognition
            result["recognition_resume_requested"] = False
            if should_resume_recognition:
                self._queue_recognition_resume("delete")
                result["recognition_resume_requested"] = True
                started = time.time()
                while time.time() - started < 8.0:
                    if self._recognition_running or self._is_gui_recognition_running():
                        result["recognition_resumed"] = True
                        break
                    time.sleep(0.2)
                else:
                    result["recognition_resumed"] = False
            return True, "User deleted.", result
        except Exception as e:
            self._api_last_error = str(e)
    ...
    def export_logs_archive(self) -> tuple[bool, str, dict]:
        """Create a ZIP containing only today's Pi local LOG file."""
        try:
            stamp = time.strftime("%Y%m%d_%H%M%S")
            today = time.strftime("%Y-%m-%d")
            temp_dir = os.path.join(tempfile.gettempdir(), "sas_pi_log_exports")
            os.makedirs(temp_dir, exist_ok=True)
            cutoff = time.time() - 3600
            for old_name in os.listdir(temp_dir):
                if not old_name.startswith("Pi_LOG_") or not old_name.endswith(".zip"):
                    continue
            ...
            zip_path = os.path.join(temp_dir, f"Pi_LOG_{stamp}.zip")

            with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zipf:
                today_log = os.path.join(LOG_DIR, f"{today}.log")
                if os.path.isfile(today_log):
                    zipf.write(today_log, os.path.join("Pi_LOG", f"{today}.log"))
                zipf.writestr(
                    "Pi_LOG/export_info.txt",
                    f"Exported at: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                    f"Device: {self._device_name()}\n"
                    f"Date: {today}\n",
                )

            return True, "Pi logs archive created.", {
                "zip_path": zip_path,
                "filename": os.path.basename(zip_path),
            }
        except Exception as e:
            return False, f"Pi log export failed: {e}", {}
    ...
    def _create_face_data_zip(self, output_zip: str) -> str:
        """Create an InsightFace face_data ZIP package.

        The package contains portable registration data:
        users.json, embeddings.npz, and registered face photos. Runtime model
        files such as buffalo_s stay on each Pi and are never exported.
        """
        with self._face_data_lock:
            return get_engine().export_to_zip(output_zip)
    ...
    def export_face_data_to_folder(self, target_folder: Optional[str] = None,
        ...
        """Export the current InsightFace face_data package to a folder.

        Write to a temporary ``.part`` file first and rename only after the ZIP
        is valid. This keeps the old fast SMB export path while preventing SAS
        from seeing half-written packages.
        """
        server_part = ""
        try:
            if target_folder:
            ...
            server_part = f"{output_zip}.part"

            self.log(f"Creating export package on server: {zip_name}", "info")
            try:
                if os.path.exists(server_part):
                    os.remove(server_part)
            except Exception:
                pass

            self._create_face_data_zip(server_part)
            if not get_engine().is_valid_zip(server_part):
                try:
                    if os.path.exists(server_part):
                        os.remove(server_part)
                except Exception:
                    pass
                raise RuntimeError("Export ZIP validation failed.")

            try:
                os.replace(server_part, output_zip)
            except Exception:
                try:
                    ...
                    pass
                try:
                    os.rename(server_part, output_zip)
                except Exception:
                    shutil.copy2(server_part, output_zip)
                    os.remove(server_part)
    ...
    def _merge_import_zip_add_new_only(self, zip_path: str) -> tuple[bool, str, dict]:
        """Merge an InsightFace face_data ZIP by employee ID."""
        ...
        try:
            with self._face_data_lock:
                result = get_engine().import_from_zip(zip_path)
            result["backup"] = backup_file
            imported_users = result.get("imported_users", [])
            name_conflicts = result.get("name_conflicts", [])
            message = (
                f"Import completed. New users: {result.get('new_users_count', 0)}, "
                f"updated users: {result.get('updated_users_count', 0)}, "
                f"embeddings added: {result.get('embeddings_added', 0)}, "
                f"photos added: {result.get('photos_added', 0)}, "
                f"name conflicts kept local: {len(name_conflicts)}."
            )
            self._refresh_dataset_ui()
            self.log(
                f"InsightFace import completed. Users={imported_users}; "
                f"embeddings_added={result.get('embeddings_added', 0)}; "
                f"duplicate_embeddings={result.get('duplicate_embeddings', 0)}; "
                f"photos_added={result.get('photos_added', 0)}; "
                f"duplicate_photos={result.get('duplicate_photos', 0)}; "
                f"name_conflicts={name_conflicts}",
                "success" if imported_users else "warn",
            )
    ...
    def sftp_send_face_data(
        ...
        try:
            ...
            remote_part = remote_file + ".part"

            try:
                sftp.remove(remote_part)
            except Exception:
                pass
            sftp.put(local_zip, remote_part)
            try:
                sftp.remove(remote_file)
            except Exception:
                pass
            sftp.rename(remote_part, remote_file)
            remote_part = ""
    ...
    def _create_multi_sftp_package(self) -> tuple[str, str]:
        ...
        if not get_engine().has_face_data():
            raise FileNotFoundError("No InsightFace face_data users/embeddings were found. Register users before sending.")
