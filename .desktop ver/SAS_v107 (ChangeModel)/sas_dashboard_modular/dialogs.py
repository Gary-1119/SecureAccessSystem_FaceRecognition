...
class SystemLockedNotification(QDialog):
    ...
    def restart_video_stream(self, video_feed_url: str = ""):
        """Restart the lock-card camera subscriber after Pi reconnects."""
        if video_feed_url:
            self.video_feed_url = str(video_feed_url or "").strip()
        self._stop_video_stream()
        self._last_frame_image = None
        ...
        try:
            ...
            self.camera_placeholder.raise_()
        except Exception:
            pass
        self._set_live_status("Connecting")
        QTimer.singleShot(300, self._start_video_stream)

    def mark_camera_unavailable(self, status: str = "Camera unavailable"):
        """Clear stale lock-card camera frame when Pi/video-feed is gone."""
        self._stop_video_stream()
        self._last_frame_image = None
        self._video_received_first_frame = False
        try:
    ...
    def _set_live_status(self, status: str):
        ...
        if status_text in {"LIVE", "CONNECTING", "RECONNECTING"}:
            ...
            if status_text in {"CAMERA UNAVAILABLE", "UNAVAILABLE", "DISCONNECTED"}:
                try:
                    self.camera_preview_label.clear()
                    self.camera_preview_label.setPixmap(QPixmap())
                    self._last_frame_image = None
...
class TrainingProgressDialog(QDialog):
    ...
    def start_animation(self):
        ...
        self.loading_timer: Optional[QTimer] = QTimer(self)
    ...
    def set_live_progress(
        ...
        if total_value > 0:
            ...
            self.ntid_value_label.setText(f"{already_value} already registered")
    ...
    def set_processing_users(self, users):
        """Update token card to show actual registered user data, not typed NTID."""
        ...
        if user_list:
            ...
            self.ntid_title_label.setText("REGISTERED USER DATA")
            self.ntid_value_label.setText(text)
        else:
    ...
    def _finish(self, success: bool, message: str):
        try:
            progress_timer = self.progress_timer
