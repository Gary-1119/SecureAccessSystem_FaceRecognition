...
class FaceApiService:
    ...
    def capture_user(self, user_id: str, mode="add", auto_capture=False, display_name: str = "", name: str = ""):
        display_name = str(display_name or name or user_id or "").strip()
        return self.request(
            "POST",
            "/capture-user",
            {
    ...
    def delete_user(self, user_id: str, display_name: str = "", name: str = ""):
        display_name = str(display_name or name or "").strip()
        return self.request(
            "POST",
            "/delete-user",
            {
    ...
    def export_face_data(self, target_folder=None):
        return self.request("POST", "/face-data/export", {"target_folder": target_folder}, timeout=300)

    def import_face_data(self, zip_path: str):
        return self.request("POST", "/face-data/import", {"zip_path": zip_path}, timeout=120)
