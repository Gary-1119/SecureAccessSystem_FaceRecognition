...
class SecureAccessDashboard(QMainWindow):
    ...
    def build_settings_page(self):
        ...
        for btn in (cancel, save):
            ...
            btn.setMinimumHeight(40 if not self.compact_mode else 34)
            btn.setMinimumWidth(96 if not self.compact_mode else 82)

        cancel.clicked.connect(self.load_settings_from_credentials)
        save.clicked.connect(self.save_settings_from_ui)

        header_actions.addWidget(cancel)
    ...
    def show_settings_message(self, title: str, message: str, success: bool = True):
    ...
    def apply_timeout_to_countdown(self, reset: bool = False):
        ...
        if hasattr(self, "ring"):
            self.ring.total_seconds = self.total_seconds
    ...
    def save_settings_from_ui(self):
        ...
        if not ntid:
            self.show_settings_message("Missing NTID", "Please enter NTID before saving settings.", success=False)
            return

        if not password:
            self.show_settings_message("Missing Password", "Please enter password before saving settings.", success=False)
            return

        if not server:
            self.show_settings_message("Missing Server Path", "Please select or enter the server path.", success=False)
            return
        ...
        self.append_system_log("Validating settings NTID/password before saving...")
        ...
        def worker():
            ...
            def done():
                if not valid:
                    self.append_system_log(f"Settings validation failed: {reason}")
                    self.show_settings_message(
                        "Validation Failed",
                        "Invalid NTID or password. Settings were not saved.",
                        success=False,
                    )
                    return
                ...
                self.append_system_log("Settings saved and validated")
                self.show_settings_message("Settings Saved", "NTID and password validated. Settings saved successfully.", success=True)
