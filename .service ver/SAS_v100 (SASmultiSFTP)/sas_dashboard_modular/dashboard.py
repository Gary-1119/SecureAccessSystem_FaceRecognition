...
class SecureAccessDashboard(QMainWindow):
    ...
    def _multi_sftp_render_progress(self, dialog, snapshot):
        ...
        footer_layout.setContentsMargins(12, 8, 12, 8)
        footer_layout.setSpacing(8)
        cancel = QPushButton("Cancel & Return")
        cancel.setObjectName("MultiSftpFooterCancel")
        cancel.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        cancel.clicked.connect(lambda: self._multi_sftp_cancel_batch(dialog, close_after=True))
    ...
    def _multi_sftp_apply_snapshot(self, dialog, snapshot):
        ...
        for job in jobs:
            ...
            if status_value == "Success":
                ...
                short = message if len(message) <= 40 else message[:37] + "..."
                display = f"Failed: {short}" if short else "Failed"
            ...
            if status_label.toolTip() != message:
        ...
        if running:
            if cancel_button is not None:
                if cancelling:
                    if state.get("close_after_cancel"):
                        if cancel_button.text() != "Closing After Stop":
                            cancel_button.setText("Closing After Stop")
                        cancel_button.setEnabled(False)
                    else:
                        if cancel_button.text() != "Close After Stop":
                            cancel_button.setText("Close After Stop")
                        cancel_button.setEnabled(True)
                    cancel_button.show()
                else:
                    if cancel_button.text() != "Cancel & Return":
                        cancel_button.setText("Cancel & Return")
                    cancel_button.setEnabled(True)
                    cancel_button.show()

            if action is not None:
                action_style_changed = action.objectName() != "MultiSftpAbortButton"
                next_text = "Cancelling..." if cancelling else "Abort Transfer"
            ...
            # Successful completion has one footer Close only: the right-side
            # primary action. Hide the left button to avoid duplicate Close.
            if cancel_button is not None:
                if has_retry:
                    if cancel_button.text() != "Back to Targets":
                        cancel_button.setText("Back to Targets")
                    cancel_button.setEnabled(True)
                    cancel_button.show()
                else:
                    cancel_button.hide()
    ...
    def _multi_sftp_cancel_batch(self, dialog, close_after: bool = False):
        ...
        # Abort may already be in flight. A later footer Cancel / × click must
        # not look stuck: it upgrades the result to "close after stop".
        ...
        if cancel_button is not None:
            if close_after:
                ...
                # Keep this available after Abort so user can choose to close
                # and return instead of waiting to inspect the final result.
                cancel_button.setText("Close After Stop")
                cancel_button.setEnabled(True)
            ...
            def done():
                ...
                if error:
                    ...
                    if live.get("progress_cancel") is not None:
                        live["progress_cancel"].setText("Cancel & Return")
                        live["progress_cancel"].setEnabled(True)
                    return
    ...
    def _multi_sftp_progress_action(self, dialog):
        ...
        if bool(snapshot.get("running", False)):
            self._multi_sftp_cancel_batch(dialog, close_after=False)
            return
