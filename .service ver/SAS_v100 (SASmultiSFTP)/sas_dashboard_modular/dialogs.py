...
class LockNotificationIcon(QWidget):
    """Small custom painted lock icon used by the desktop lock notification.

    The icon is drawn with QPainter rather than an emoji/font glyph so it is
    consistent on older Windows computers that may not have the same emoji
    font fallback as newer laptops.
    """
    ...
    def paintEvent(self, event):
        ...
        painter.setBrush(QColor("#3D0B15"))
        painter.drawEllipse(self.rect())

        # Padlock shackle.
        painter.setPen(QPen(QColor("#FF6B7A"), 2.2))
        painter.setBrush(Qt.BrushStyle.NoBrush)
        shackle = QRectF(self.width() * 0.33, self.height() * 0.20, self.width() * 0.34, self.height() * 0.36)
        painter.drawArc(shackle, 25 * 16, 130 * 16)

        # Padlock body.
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QColor("#FF5A6B"))
        body = QRectF(self.width() * 0.27, self.height() * 0.48, self.width() * 0.46, self.height() * 0.31)
        painter.drawRoundedRect(body, 3.0, 3.0)

        # Keyhole.
        painter.setBrush(QColor("#2B0710"))
        painter.drawEllipse(QRectF(self.width() * 0.46, self.height() * 0.57, self.width() * 0.08, self.height() * 0.10))
        painter.drawRoundedRect(QRectF(self.width() * 0.485, self.height() * 0.64, self.width() * 0.03, self.height() * 0.08), 1.0, 1.0)


class SystemLockedNotification(QDialog):
    """Small non-modal lock notification with a read-only Pi camera preview.

    The notification deliberately does not own, restore, or activate the SAS
    dashboard.  SAS can remain minimized while the user sees the normal
    Windows desktop.  The preview opens a second *read-only* subscription to
    the Pi ``/video-feed`` MJPEG endpoint; it does not start/stop recognition,
    capture images, or control the physical camera.
    """
    ...
    def __init__(
        ...
        # A dedicated viewer is used while the dashboard is minimized.  It is
        # intentionally separate from the dashboard preview and exits cleanly
        ...
        # Keep this as a compact desktop notification while giving the live
        # camera preview a true 1:1 square viewport.  The camera image is
        # centre-cropped by _display_frame_image() to fill this square.
        self._notification_width = 360 if not compact_mode else 336
        self._camera_panel_side = 224 if not compact_mode else 204
        self.resize(
            self._notification_width,
            365 if not compact_mode else 337,
        )
    ...
    def show_at_bottom_right(self):
        ...
        # The card layout is finalized on the next Qt turn; place the direct
        # camera-preview overlays after that layout pass.
        QTimer.singleShot(0, self._position_camera_overlays)

        # Start after the notification has a final geometry.  This makes the
        # first frame scale correctly and keeps the GUI thread responsive.
    ...
    def _start_video_stream(self):
        ...
        if not self.video_feed_url:
            self._set_live_status("Camera URL unavailable")
            return

        if cv2 is None:
            self._set_live_status("OpenCV unavailable")
            return
    ...
    def _video_worker(self):
        """Read the Pi MJPEG stream on a background thread.

        The Pi already publishes the latest camera frame at ``/video-feed``.
        This worker is only another visual subscriber.  It never sends
        recognition, capture, train, or stop commands.
        """
        capture = None
        stream_failed = False
        ...
        try:
            ...
            while self._video_stream_running and not self._video_stream_stop.is_set():
                ...
                # Keep the compact notification responsive and avoid decoding/
                # painting more frames than the small preview can display.
                now = time.monotonic()
            ...
            if not self._video_stream_stop.is_set() and not stream_failed:
                try:
                    self.live_status_changed.emit("Camera unavailable")
                except Exception:
                    pass
    ...
    def _set_live_status(self, status: str):
        ...
        if status_text == "LIVE":
            dot_color = "#FF5A6B"
            text_color = "#FF7B88"
        elif status_text in {"CONNECTING", "RECONNECTING"}:
            dot_color = "#D6A84E"
            text_color = "#E0BD71"
        else:
            dot_color = "#747878"
    ...
    def build_ui(self):
        ...
        card = QFrame()
        card.setObjectName("SystemLockedNotificationCard")
        card.setStyleSheet("""
            QFrame#SystemLockedNotificationCard {
                background: #1C1B1B;
                border: 1px solid rgba(116, 120, 120, 0.28);
                border-radius: 12px;
            }
        """)
        ...
        layout = QVBoxLayout(card)
        horizontal_margin = 16 if not self.compact_mode else 14
        layout.setContentsMargins(
            horizontal_margin,
            14 if not self.compact_mode else 12,
            horizontal_margin,
            14 if not self.compact_mode else 12,
        )
        layout.setSpacing(7 if not self.compact_mode else 5)

        header = QHBoxLayout()
        header.setContentsMargins(0, 0, 0, 0)
        header.setSpacing(8)

        icon = LockNotificationIcon(card, 32 if not self.compact_mode else 29)
        ...
        message = QLabel("Access Suspended. Scan Face To Unlock")
        message.setWordWrap(True)
        ...
        self.camera_panel = QFrame(card)
        self.camera_panel.setObjectName("LockedNotificationCameraPanel")
        # Deliberately fixed to a square so the lock notification has a clear
        # face-scanning viewport without becoming a full-screen overlay.
        self.camera_panel.setFixedSize(
            self._camera_panel_side,
            self._camera_panel_side,
        )
        ...
        self.camera_panel.setStyleSheet("""
            QFrame#LockedNotificationCameraPanel {
                background: #000000;
                border: 1px solid rgba(116, 120, 120, 0.34);
                border-radius: 8px;
            }
        """)
        ...
        self.camera_preview_label.setStyleSheet(
            "background: #000000; border-radius: 7px; color: #858383;"
        )
        self.camera_preview_label.setText("Connecting to Pi camera...")

        # Four red biometric frame corners mirror the supplied design.  They are
        # visual-only and do not add controls or input handling.
        corner_color = "#FF5A6B"
        ...
        self.corner_top_left.setStyleSheet(
            f"border-top: 2px solid {corner_color}; border-left: 2px solid {corner_color};"
        )
        self.corner_top_right.setStyleSheet(
            f"border-top: 2px solid {corner_color}; border-right: 2px solid {corner_color};"
        )
        self.corner_bottom_left.setStyleSheet(
            f"border-bottom: 2px solid {corner_color}; border-left: 2px solid {corner_color};"
        )
        self.corner_bottom_right.setStyleSheet(
            f"border-bottom: 2px solid {corner_color}; border-right: 2px solid {corner_color};"
        )

        self.scan_line = QFrame(self.camera_panel)
        self.scan_line.setStyleSheet(
            "background: rgba(255, 90, 107, 0.26); border: none;"
        )

        self.scan_hint = QLabel("READY FOR SCAN", self.camera_panel)
        ...
        self.live_status_dot.setFixedSize(7, 7)

        self.live_status_label = QLabel("CONNECTING", self.camera_panel)
        self.live_status_label.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
    ...
    def _position_camera_overlays(self):
        ...
        corner_size = 17 if not self.compact_mode else 15
        inset = 1
        ...
        self.scan_line.setGeometry(9, max(12, height // 2), max(1, width - 18), 2)
        self.scan_hint.setGeometry(0, max(0, height // 2 - 12), width, 24)

        label_width = 74 if not self.compact_mode else 68
        label_x = max(16, width - label_width - 10)
        label_y = max(4, height - 19)
        self.live_status_dot.move(label_x, label_y + 4)
        self.live_status_label.setGeometry(label_x + 11, label_y, label_width, 14)
