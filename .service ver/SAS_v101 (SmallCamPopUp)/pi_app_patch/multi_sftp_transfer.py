...
class MultiSftpTransferManager:
    ...
    def cancel_batch(self, batch_id: str) -> Tuple[bool, str, Dict[str, Any]]:
        with self._lock:
            ...
            if not batch.running:
                return True, "Transfer has already stopped.", self._snapshot_unlocked(batch)

            if batch.cancel_event.is_set():
                return True, "Cancellation is already in progress.", self._snapshot_unlocked(batch)
            ...
            return (
                True,
                "Cancellation requested. Active uploads will stop at the next progress update.",
                self._snapshot_unlocked(batch),
            )
    ...
    def _run_batch(self, batch_id: str, target_hosts: List[str], create_zip: bool) -> None:
        ...
        try:
            ...
            if batch.cancel_event.is_set():
                # Important: cancellation can happen while the face-data ZIP is
                # still being prepared, before any SFTP worker starts.  The old
                # code returned here without finalising the batch, leaving
                # batch.running=True forever even though every target showed
                # Cancelled.  Finish the batch so API clients can close/clean up.
