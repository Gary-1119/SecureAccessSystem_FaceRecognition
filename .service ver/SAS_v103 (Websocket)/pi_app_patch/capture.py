...
def capture_faces(person_name, mode="add", save_base="dataset",
    ...
    try:
        ...
        while True:
            ...
            # Use the same persisted orientation as recognition and the API feed.
            # Images saved for training therefore match what the operator sees.
            ...
            if auto_capture:
                ...
                if (
                    len(faces) == 1
                    and auto_count < AUTO_LIMIT
                    and (now - last_auto_time) >= AUTO_COOLDOWN
                ):
                    img_path = f"{person_folder}/image_{img_counter}.jpg"

                    # Save directly because current frame colour is already correct for cv2.imwrite()
                    save_frame = frame.copy()

                    # If frame has 4 channels, remove the extra channel only
                    if len(save_frame.shape) == 3 and save_frame.shape[2] == 4:
            ...
            if frame_callback:
                ...
                if result == "capture":
                    ...
                    if len(faces) == 1:
                        # Save directly because the frame is already BGR for cv2.imwrite()
                        save_frame = frame.copy()
                        
                        # If frame has 4 channels, remove the alpha/extra channel only
                        if len(save_frame.shape) == 3 and save_frame.shape[2] == 4:
                            save_frame = save_frame[:, :, :3]
