...
class SecureAccessDashboard(QMainWindow):
    def __init__(self):
        ...
        self._suppress_lock_overlay = False
        ...
        self.locked_alert_dialog = None
        self.lock_overlay_blur = None
        self.ui_scale = 1.0
    ...
    def has_restricted_tab_access(self) -> bool:
        """Admin access is required for restricted tabs."""
        user = (self.current_user or "").strip().upper()
        role = (self.current_user_role or "").strip().lower()

        if user == "EMERGENCY" or role == "override":
            return True
    ...
    # --------------------------------------------------------
    # Locked alert overlay
    # --------------------------------------------------------
    def show_locked_alert_overlay(self):
        """Show dark lock alert overlay. It has no close button and only disappears after unlock."""
        if getattr(self, "_suppress_lock_overlay", False):
            return

        if hasattr(self, "locked_alert_dialog") and self.locked_alert_dialog is not None:
            if self.locked_alert_dialog.isVisible():
                return

        root = self.centralWidget()
        if root is not None:
            old_effect = root.graphicsEffect()
            if old_effect is None:
                self.lock_overlay_blur = QGraphicsBlurEffect(self)
                self.lock_overlay_blur.setBlurRadius(0)
                root.setGraphicsEffect(self.lock_overlay_blur)

                self._lock_blur_anim = QPropertyAnimation(self.lock_overlay_blur, b"blurRadius", self)
                self._lock_blur_anim.setDuration(240)
                self._lock_blur_anim.setStartValue(0)
                self._lock_blur_anim.setEndValue(6)
                self._lock_blur_anim.setEasingCurve(QEasingCurve.Type.OutCubic)
                self._lock_blur_anim.start()

        self.locked_alert_dialog = LockedAlertDialog(self, self.compact_mode)
        self.locked_alert_dialog.setGeometry(self.geometry())
        self.locked_alert_dialog.show()
        self.locked_alert_dialog.raise_()

    def hide_locked_alert_overlay(self):
        """Hide lock overlay and restore background when valid face unlocks."""
        if hasattr(self, "locked_alert_dialog") and self.locked_alert_dialog is not None:
            self.locked_alert_dialog.closeEvent = lambda event: event.accept()
            self.locked_alert_dialog.accept()
            self.locked_alert_dialog = None

        root = self.centralWidget()
        if root is not None:
            effect = root.graphicsEffect()
            if effect is not None and getattr(self, "lock_overlay_blur", None) is effect:
                self._lock_blur_out_anim = QPropertyAnimation(effect, b"blurRadius", self)
                self._lock_blur_out_anim.setDuration(200)
                self._lock_blur_out_anim.setStartValue(cast(QGraphicsBlurEffect, effect).blurRadius())
                self._lock_blur_out_anim.setEndValue(0)
                self._lock_blur_out_anim.setEasingCurve(QEasingCurve.Type.OutCubic)

                def clear_effect():
                    effect.setEnabled(False)
                    effect.deleteLater()
                    if root.graphicsEffect() is effect:
                        root.setGraphicsEffect(cast(QGraphicsEffect, None))  # type: ignore[arg-type]

                self._lock_blur_out_anim.finished.connect(clear_effect)
                self._lock_blur_out_anim.start()

    def resize_locked_alert_overlay(self):
        if hasattr(self, "locked_alert_dialog") and self.locked_alert_dialog is not None:
            if self.locked_alert_dialog.isVisible():
                self.locked_alert_dialog.setGeometry(self.geometry())
    ...
    def show_account_menu(self):
        ...
        if self.current_user:
            ...
            if display_user == "EMERGENCY" or str(role).lower() == "override":
                display_user = "ADMIN"
                role = "Admin"
    ...
    def build_settings_page(self):
        ...
        grid.addWidget(self.security_options_card(), 1, 1)
    ...
    def apply_lock_state(self):
        ...
        if self.is_locked:
            self.restore_main_window_for_attention("system locked")
            QTimer.singleShot(80, self.show_locked_alert_overlay)
        else:
            self.hide_locked_alert_overlay()
    ...
    def write_sas_log(self, message: str):
        """Write SAS unlock log to BOTH local SAS_LOG and configured server_path/SAS_LOG.

        SAS_LOG stores unlock events:
        - face unlock
        - emergency unlock
        - manual unlock
        """
        if not str(message).startswith("UNLOCKED"):
            return
    ...
    def enter_first_launch_setup_mode(self):
        ...
        self.apply_lock_state()
        self.update_account_ui()
        self.append_system_log("FIRST_LAUNCH_SETUP: Missing server credentials. Lock overlay suppressed until setup is saved.")
    ...
    def load_settings_from_credentials(self):
        ...
        if not self.server_configured:
            self._suppress_lock_overlay = True
    ...
    def sync_settings_to_pi_async(self, ntid: str, password: str, server: str, auto_capture: bool):
        ...
        def worker():
            ok = self._face_api_sync_settings_to_pi(ntid, password, server, auto_capture)
    ...
    def save_settings_from_ui(self):
        ...
        def worker():
            ...
            def done():
                ...
                if str(getattr(self, "pi_api_host", "") or "").strip():
                    self.sync_settings_to_pi_async(ntid, password, server, auto_capture)
                else:
    ...
    def _handle_successful_login(self, ntid: str, is_admin: bool, role_msg: str, debug_text: str, dialog):
    ...
    def _handle_login_from_popup(self, ntid: str, password: str, dialog):
        ...
        if self._is_hardcoded_admin_login(ntid, password):
            ...
            self._handle_successful_login("admin", True, "Built-in local Admin login successful.", debug_text, dialog)
            return
    ...
    def update_account_ui(self):
        if hasattr(self, "login_btn"):
            if self.current_user:
                ...
                # Hotkey/emergency unlock should appear as Admin in the UI.
                # No "Emergency" and no "Override" wording for supervisor requirement.
                if display_user == "EMERGENCY" or str(display_role).lower() == "override":
                    display_user = "ADMIN"
                    display_role = "Admin"
            ...
            try:
                ...
                def done():
                    try:
                        ...
                        if isinstance(data, dict) and "auto_capture" in data and hasattr(self, "auto_capture_toggle"):
                            self.auto_capture_enabled = bool(data.get("auto_capture", False))
    ...
    def _face_api_sync_settings_to_pi(self, ntid: str, password: str, server: str, auto_capture: bool):
        ...
        try:
            ...
            def ui_done():
                ...
                self.append_face_log(
                    ...
                    f"AUTO_CAPTURE={state} | STATE_FILE=recognition_result.json"
                )
    ...
    def _face_api_sftp_send(self):
        dialog, widgets = self.create_sftp_transfer_dialog()
        ...
        def do_transfer():
            ...
            remote_dir = self.normalize_sftp_pending_path(folder)

            try:
                port = int(widgets["port"].text().strip() or "22")
            except Exception:
                self.set_sftp_status(widgets["status"], "Port must be a number. Default SFTP port is 22.", "error")
                return

            if not host:
                self.set_sftp_status(widgets["status"], "Receiver Host / IP is required.", "error")
                return
            if not username:
                self.set_sftp_status(widgets["status"], "Username is required.", "error")
                return
            if not password:
                self.set_sftp_status(widgets["status"], "Password is required.", "error")
                return
            if not remote_dir:
                self.set_sftp_status(widgets["status"], "Receiver folder path is required.", "error")
                return

            widgets["pending_path"].setText(remote_dir)
            self.start_transfer_button_loading(widgets["transfer"], "Transfer")
            self.set_sftp_status(widgets["status"], f"Connecting to {host}:{port} and sending package...", "normal")
            ...
            def worker():
                try:
                    ...
                    message = data.get("message", "Transfer completed.")
                    remote_file = data.get("remote_file", remote_dir)

                    def done():
                        self.show_sftp_transfer_success(dialog, widgets, message, remote_file)
                        self.append_face_log(f"SFTP_SEND: {message} | {remote_file}")
                    ...
                    def fail(err=err):
                        self.show_transfer_button_failed(widgets["transfer"], "Transfer  →")
                        self.set_sftp_status(widgets["status"], f"SFTP transfer failed: {err}", "error")
                        self.append_face_log(f"SFTP_SEND_FAILED: {err}")
        ...
        # v190: Pressing Enter inside any SFTP field must use the exact same
        # transfer workflow as clicking Transfer, including the in-popup
        # loading/sending status and disabled controls while sending.
        def trigger_sftp_transfer_from_enter():
            try:
                if widgets["transfer"].isEnabled():
                    do_transfer()
            except Exception as e:
                status = getattr(self, "sftp_status_label", None) or QLabel("")
                status.setText(f"Transfer trigger failed: {e}")
                status = getattr(self, "sftp_status_label", None) or QLabel("")
        ...
        # QDialog normally treats Enter as accept/close. Use an event filter so
        # Enter stays inside this popup and triggers the same transfer workflow.
        try:
        ...
        try:
            enter_filter = SftpEnterKeyFilter(trigger_sftp_transfer_from_enter, dialog)
            setattr(dialog, "_sftp_enter_filter", enter_filter)
            dialog.installEventFilter(enter_filter)
            for input_name in ("host", "port", "username", "password", "folder"):
                widgets[input_name].installEventFilter(enter_filter)
        except Exception:
            pass

        # Keep returnPressed connections as a fallback for line edits.
        for input_name in ("host", "port", "username", "password", "folder"):
            try:
                widgets[input_name].returnPressed.connect(trigger_sftp_transfer_from_enter)
            except Exception:
                pass
...
"""
    ...
    def resizeEvent(self, event):
        ...
        self.resize_locked_alert_overlay()
    ...
    def apply_styles(self):
        ...
        self.setStyleSheet(f"""
            ...
            #LockedAlertCard {{
                ...
                border: 1px solid rgba(220, 20, 60, 0.36);
                border-radius: 32px;
            }}
            ...
            #LockedAlertIcon {{
                ...
                font-size: {38 if not self.compact_mode else 32}px;
                font-weight: 900;
            }}
            ...
            #LockedAlertStatusCard {{
                background: #201F1F;
                border: 1px solid rgba(92, 63, 63, 0.65);
                border-radius: 16px;
            }}

            #LockedAlertStatusLabel {{
                color: #AC8888;
                font-size: {10 if not self.compact_mode else 9}px;
                font-weight: 900;
                letter-spacing: 2px;
            ...
            #LockedAlertScanButton:hover {{
                background: #18181B;
                border: 1px solid rgba(220, 20, 60, 0.50);
            }}

            #LockedAlertScanButton:pressed {{
                background: #252525;
                padding-top: 2px;
            }}

            #LockedAlertManualButton {{
                background: transparent;
                color: #C8C6C5;
                border: none;
                font-size: {11 if not self.compact_mode else 9}px;
