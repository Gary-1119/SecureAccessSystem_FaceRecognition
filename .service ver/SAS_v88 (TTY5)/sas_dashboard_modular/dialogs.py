...
class LockedAlertDialog(QDialog):
    """Full-window lock alert overlay.

    No close button and no X. It is dismissed only when the dashboard unlocks.
    """
    ...
    def __init__(self, parent=None, compact_mode: bool = False):
        ...
        self.setObjectName("LockedAlertDialog")
        self.setWindowFlags(
            Qt.WindowType.Dialog
            | Qt.WindowType.FramelessWindowHint
            | Qt.WindowType.WindowStaysOnTopHint
    ...
    def reject(self):
        """Disable Esc / window close while locked."""
        return

    def closeEvent(self, event):
        """Prevent manual closing while locked."""
        event.ignore()
    ...
    def build_ui(self):
        root = QFrame()
        root.setObjectName("LockedOverlayRoot")
        ...
        card = QFrame()
        card.setObjectName("LockedAlertCard")
        card.setMaximumWidth(520 if not self.compact_mode else 460)
        card.setMinimumWidth(430 if not self.compact_mode else 380)
        ...
        card_layout.setSpacing(18 if not self.compact_mode else 14)
        card_layout.setAlignment(Qt.AlignmentFlag.AlignCenter)

        icon = QLabel("🛡")
        icon.setObjectName("LockedAlertIcon")
        icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        icon.setFixedSize(82 if not self.compact_mode else 70, 82 if not self.compact_mode else 70)

        title = QLabel("SYSTEM LOCKED")
        title.setObjectName("LockedAlertTitle")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)

        desc = QLabel("Access suspended. Biometric authentication required to restore system operations.")
        desc.setObjectName("LockedAlertDesc")
        desc.setAlignment(Qt.AlignmentFlag.AlignCenter)
        desc.setWordWrap(True)
        desc.setMaximumWidth(360)

        status_card = QFrame()
        status_card.setObjectName("LockedAlertStatusCard")
        status_layout = QHBoxLayout(status_card)
        status_layout.setContentsMargins(18, 14, 18, 14)

        status_label = QLabel("STATUS")
        status_label.setObjectName("LockedAlertStatusLabel")

        status_value = QLabel("●  SECURE LOCKDOWN")
        status_value.setObjectName("LockedAlertStatusValue")
        status_value.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)

        status_layout.addWidget(status_label)
        status_layout.addStretch()
        status_layout.addWidget(status_value)

        scan_btn = QPushButton("  Scan Face to Unlock")
        scan_btn.setObjectName("LockedAlertScanButton")
        scan_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        scan_btn.setMinimumHeight(54 if not self.compact_mode else 48)
        scan_btn.clicked.connect(self.scan_clicked)

        manual = QPushButton("⚙  MANUAL OVERRIDE")
        manual.setObjectName("LockedAlertManualButton")
        manual.setCursor(Qt.CursorShape.PointingHandCursor)
        manual.clicked.connect(self.manual_clicked)

        card_layout.addWidget(icon, alignment=Qt.AlignmentFlag.AlignCenter)
        card_layout.addWidget(title)
        card_layout.addWidget(desc)
        card_layout.addWidget(status_card)
        card_layout.addWidget(scan_btn)
        card_layout.addWidget(manual, alignment=Qt.AlignmentFlag.AlignCenter)

        layout.addWidget(card, alignment=Qt.AlignmentFlag.AlignCenter)

    def scan_clicked(self):
        parent = self.parent()
        append_log = getattr(parent, "append_system_log", None)
        if callable(append_log):
            append_log("Locked overlay: waiting for face scan to unlock.")

    def manual_clicked(self):
        parent = self.parent()
        open_login = getattr(parent, "open_login_popup", None)
        if callable(open_login):
            open_login()
