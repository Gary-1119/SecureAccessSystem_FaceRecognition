...
class SystemLockedNotification(QDialog):
    """Small non-modal desktop notification shown while SAS input locking is active.

    It deliberately does not own the dashboard window.  That lets SAS remain
    minimized in the taskbar while the user still sees normal Windows desktop
    applications behind this status notification.
    """
    ...
    def __init__(self, compact_mode: bool = False):
        ...
        self.resize(360 if not compact_mode else 336, 124 if not compact_mode else 116)
        self.build_ui()
    ...
    def build_ui(self):
        ...
        layout = QVBoxLayout(card)
        layout.setContentsMargins(16 if not self.compact_mode else 14, 14 if not self.compact_mode else 12, 16 if not self.compact_mode else 14, 14 if not self.compact_mode else 12)
