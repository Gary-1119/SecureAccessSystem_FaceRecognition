...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        # Apply fullscreen after the widgets have been created and mapped.
        # This is important for a bare xinit session on TTY5: there is no
        # desktop window manager to maximize a normal Tkinter top-level window.
        if self._service_fullscreen:
            self.root.after_idle(self._apply_tty5_fullscreen)
    ...
    def _build_controls(self, parent):
        ...
        tk.Label(top_row, text="Selected NTID", font=self.font_label, bg=CARD, fg=TEXT_PRI).pack(side="left")
        tk.Label(top_row, text="Capture and delete use this ID", font=self.font_small, bg=CARD, fg=TEXT_HINT).pack(side="left", padx=(8, 0))
    ...
    def _build_right_panel(self, parent):
        ...
        tk.Label(header, text="Trained Users", font=("DejaVu Sans", 14, "bold"), bg=CARD, fg=TEXT_PRI).pack(side="left")
    ...
    def _open_settings(self):
        ...
        auto_row = tk.Frame(conn_inner, bg=CARD_SOFT)
        ...
        auto_box.pack(side="left", padx=(2, 8))
        ...
        def toggle_auto_check(event=None):
            auto_var.set(not auto_var.get())
            draw_auto_check()

        auto_box.bind("<Button-1>", toggle_auto_check)
        auto_label.bind("<Button-1>", toggle_auto_check)
        ...
        # Auto-capture may have been changed from Windows SAS.
        # Update the popup toggle from settings.json on every open.
        self.auto_capture_enabled = self._coerce_bool(settings.get("auto_capture", self.auto_capture_enabled), default=False)
        auto_var.set(self.auto_capture_enabled)
        draw_auto_check()
        ...
        def save():
            ...
            try:
                ...
                self.auto_capture_enabled = self._coerce_bool(auto_var.get(), default=False)
        ...
        if self.current_user:
            ...
            tk.Label(
                transfer_inner,
                text="Export or import encodings.pickle and dataset through the SMB server path. Existing IDs are checked and only new data is added.",
                font=self.font_small,
                bg=CARD_SOFT,
                fg=TEXT_HINT,
                wraplength=430,
    ...
    def _load_pc_path(self):
        ...
        try:
            ...
            self.auto_capture_enabled = self._coerce_bool(data.get("auto_capture", False), default=False)
    ...
    def _login(self):
        ntid = self._ask_ntid_popup("Login", "Enter NTID")
        if not ntid:
            return
        ...
        if not self._validate_ntid_in_ad(ntid):
            self._styled_alert("Login Failed", "Invalid NTID.", "error")
        ...
        if not self.admin_list:
            ...
            self._styled_alert("Admin Registered", f"{ntid} is now the first admin.", "success")
        ...
        if ntid in self.admin_list:
            ...
            self._styled_alert("Login Success", "Admin access granted.", "success")

            # Ensure no popup/menu blocks the main UI after login
            try:
                grabbed = self.root.grab_current()
                if grabbed is not None:
                    grabbed.grab_release()
    ...
    def _api_update_latest_frame(self, frame):
        """Store the latest camera frame for /video-feed."""
        try:
            if hasattr(self, "face_service"):
    ...
    def _api_remove_user_from_encodings(self, user_id):
        ...
        backup_file = f"encodings_backup_before_api_delete_{target}_{time.strftime('%Y%m%d_%H%M%S')}.pickle"
    ...
    def _api_capture_user_request(self, user_id, mode="add"):
        ...
        def frame_cb(frame):
            ...
            return "stop" if self._stop_capture_event.is_set() else None
        ...
        return True, "Capture started. It will auto-capture up to 10 photos.", {
            "user_id": user_id,
            "mode": mode
        }
    ...
    def export_face_data(self):
        ...
        tk.Label(
            title_box,
            text="Export encodings.pickle and dataset to server path.",
            font=self.font_small,
            bg=CARD,
            fg=TEXT_HINT
    ...
    def list_server_face_data_zips(self, username=None, password=None, pc_save_path=None):
        ...
        try:
            ...
            for name in os.listdir(mount_point):
                ...
                # Validate export format quickly: must contain encodings.pickle.
                try:
    ...
    def send_face_data_to_pi(self):
        win, shell = self._modern_popup_shell("Send Face Data", 620, 720)
        ...
        tk.Label(
            title_box,
            text="Send Face Data to Another Pi",
            font=("DejaVu Sans", 15, "bold"),
            bg=CARD,
            fg=TEXT_PRI
        ).pack(anchor="w")
        ...
        tk.Label(
            title_box,
            text="Upload this Pi's face dataset to another Pi's pending receive folder.",
            font=self.font_small,
            bg=CARD,
            fg=TEXT_HINT,
            wraplength=500,
        ...
        form_canvas = tk.Canvas(shell, bg=CARD, highlightthickness=0, bd=0)
        form_canvas.pack(fill="both", expand=True, padx=24, pady=(6, 0))
        form_canvas.configure(yscrollcommand="")

        form = tk.Frame(form_canvas, bg=CARD)
        form_window = form_canvas.create_window((0, 0), window=form, anchor="nw")

        def _form_config(event=None):
        ...
        form.bind("<Configure>", _form_config)
        form_canvas.bind("<Configure>", _form_config)

        def _form_mousewheel(event):
            try:
                if event.num == 4:
        ...
        form_canvas.bind("<MouseWheel>", _form_mousewheel)
        form_canvas.bind("<Button-4>", _form_mousewheel)
        form_canvas.bind("<Button-5>", _form_mousewheel)

        host_entry = self._field_box(form, "Receiver Host/IP")
        port_entry = self._field_box(form, "Port", default="22")
        user_entry = self._field_box(form, "Receiver Username")
        pwd_entry = self._field_box(form, "Receiver Password", show="*")
        remote_entry = self._field_box(
            form,
            "Receiver Pending Folder",
            default="/home/<username>/New/received_face_data/pending"
        )
        ...
        def do_send():
            ...
            remote_dir = remote_entry.get().strip().replace("\\", "/").rstrip("/")

            if not remote_dir:
                self._styled_alert(
                    "Missing Remote Folder",
                    "Please enter the receiver pending folder path.",
                    "warning",
                    parent=win
                )
                return
            ...
            try:
                ...
                status_lbl.config(text="Uploading face data...", fg=WARNING)
                status_lbl.update_idletasks()
    ...
    def delete_user_from_encodings(self, username):
        ...
        try:
            # Backup first before modifying encodings.pickle
    ...
    def delete_user(self):
        ...
        if not self._styled_confirm(
            ...
            "A backup of encodings.pickle will be created."
        ):
            return
