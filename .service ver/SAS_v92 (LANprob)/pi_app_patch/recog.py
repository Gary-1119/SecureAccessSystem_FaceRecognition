...
def _write_json_atomic(path, data):
    """Write JSON atomically so readers never observe a partial local file."""
...
def _write_server_result_if_fresh(data, created_monotonic):
    """Write through a temporary SMB file, then replace only while still fresh."""
    server_file = os.path.join(SERVER_PATH, RECOGNITION_RESULT_FILE)
    temp_file = f"{server_file}.tmp-{os.getpid()}-{threading.get_ident()}"

    try:
        # The temp file is harmless if a network failure interrupts this call.
