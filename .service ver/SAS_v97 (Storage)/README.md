## Latest patch: SAS v106 — Hotkey Desktop-Only Unlock and Audit Trail

### Changed
- **Ctrl + Shift + Alt** now releases the locked Windows desktop only. It no longer creates an Emergency/Admin-style SAS session, opens Settings, opens Face Recognition, restores SAS, or bypasses the normal Admin login requirement.
- Hotkey unlock now behaves like a workstation unlock: SAS stays minimized/backgrounded, the Login button remains in the normal guest state, and an Admin must sign in before opening **Settings** or **Face Recognition**.
- Removed the former Emergency/Override access bypass from restricted-tab authorization.
- Expanded **SAS_LOG** into a general audit trail. It now records:
  - Face, manual, and emergency-hotkey unlocks
  - Hotkey workstation account context (not treated as authenticated identity)
  - Successful and failed SAS login attempts
  - Restricted Settings / Face Recognition page access
  - Settings Save & Connect actions with the authenticated actor
  - Admin add/remove actions with the authenticated actor
  - Explicit logout actions
- Passwords are not written to SAS_LOG.

### Files changed
```text
Modified: sas_dashboard_modular/services/runtime_lock_service.py
- Changed the emergency hotkey from an Admin-style override session to a desktop-only unlock.
- Clears SAS user/admin state, prevents restricted-page navigation, and writes a detailed hotkey audit record.

Modified: sas_dashboard_modular/dashboard.py
- Removed Emergency/Override restricted-tab bypasses.
- Added SAS_LOG audit records for logins, restricted-page access, settings saves, Admin-list changes, and logout.
- Updated SAS_LOG writer so these non-unlock audit events are persisted locally and to the configured server path.

Modified: README.md
- Added SAS v106 release notes and audit behavior.
```
...
- Lock notification camera panel
- Mouse / keyboard block implementation
- Recognition, API, SFTP, import/export, and dataset logic
```
