...
from recog import (
    start_recognition,
    queue_server_recognition_result,
...
class FaceService:
    ...
    def _reset_recognition_result_file(self) -> None:
        """Reset local state immediately and queue a safe SMB state asynchronously.

        The local file is always available on the Pi. The server copy is only a
        delivery message for Windows SAS and must never block Pi camera/UI work.
        """
        data = {
            "detected": False,
            "user_id": None,
        ...
        # The publisher performs SMB reachability checks, drops stale offline
        # results, and resets remote state safely after a reconnect.
