...
def create_api_app(gui_app):
    ...
    # --------------------------------------------------------
    # FACE DATA TRANSFER - MULTI-PI SFTP TARGETS AND BATCHES
    # --------------------------------------------------------
    @api_app.route("/face-data/sftp-targets", methods=["GET"])
    def api_list_sftp_targets():
        if not has_method("list_sftp_targets"):
            return fail("Multi-Pi SFTP target service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.list_sftp_targets()
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not list SFTP targets: {e}", status=500)

    @api_app.route("/face-data/sftp-targets", methods=["POST"])
    def api_add_sftp_target():
        if not has_method("add_sftp_target"):
            return fail("Multi-Pi SFTP target service not implemented.", status=501)
        payload = request.get_json(silent=True) or {}
        hostname = str(payload.get("hostname", payload.get("host", ""))).strip()
        if not hostname:
            return fail("Missing hostname.")
        try:
            ok_state, message, extra = gui_app.add_sftp_target(hostname)
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not add SFTP target: {e}", status=500)

    @api_app.route("/face-data/sftp-targets/<string:hostname>", methods=["DELETE"])
    def api_remove_sftp_target(hostname):
        if not has_method("remove_sftp_target"):
            return fail("Multi-Pi SFTP target service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.remove_sftp_target(hostname)
            return ok(message, **extra) if ok_state else fail(message, status=404)
        except Exception as e:
            return fail(f"Could not remove SFTP target: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-config", methods=["GET"])
    def api_multi_sftp_config():
        if not has_method("get_multi_sftp_transfer_defaults"):
            return fail("Multi-Pi SFTP configuration service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.get_multi_sftp_transfer_defaults()
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not read multi-Pi SFTP configuration: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-send", methods=["POST"])
    def api_start_multi_sftp_send():
        if not has_method("start_multi_sftp_send"):
            return fail("Multi-Pi SFTP transfer service not implemented.", status=501)
        payload = request.get_json(silent=True) or {}
        hostnames = payload.get("hostnames", payload.get("targets", []))
        if not isinstance(hostnames, list):
            return fail("hostnames must be a list.")
        # The Pi owns the fixed four-worker policy. A smaller API value may be
        # used for test environments, but callers cannot exceed four workers.
        max_workers = payload.get("max_workers", 4)
        try:
            ok_state, message, extra = gui_app.start_multi_sftp_send(hostnames, max_workers=max_workers)
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not start multi-Pi SFTP transfer: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-send/<string:batch_id>", methods=["GET"])
    def api_multi_sftp_status(batch_id):
        if not has_method("get_multi_sftp_transfer_status"):
            return fail("Multi-Pi SFTP transfer service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.get_multi_sftp_transfer_status(batch_id)
            return ok(message, **extra) if ok_state else fail(message, status=404)
        except Exception as e:
            return fail(f"Could not read multi-Pi SFTP transfer status: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-send/<string:batch_id>/retry-failed", methods=["POST"])
    def api_retry_multi_sftp_failed(batch_id):
        if not has_method("retry_failed_multi_sftp_transfer"):
            return fail("Multi-Pi SFTP retry service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.retry_failed_multi_sftp_transfer(batch_id)
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not retry multi-Pi SFTP transfer: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-send/<string:batch_id>/cancel", methods=["POST"])
    def api_cancel_multi_sftp(batch_id):
        if not has_method("cancel_multi_sftp_transfer"):
            return fail("Multi-Pi SFTP cancellation service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.cancel_multi_sftp_transfer(batch_id)
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not cancel multi-Pi SFTP transfer: {e}", status=500)

    @api_app.route("/face-data/sftp-multi-send/<string:batch_id>/cleanup", methods=["POST"])
    def api_cleanup_multi_sftp(batch_id):
        if not has_method("cleanup_multi_sftp_transfer"):
            return fail("Multi-Pi SFTP cleanup service not implemented.", status=501)
        try:
            ok_state, message, extra = gui_app.cleanup_multi_sftp_transfer(batch_id)
            return ok(message, **extra) if ok_state else fail(message)
        except Exception as e:
            return fail(f"Could not clean up multi-Pi SFTP package: {e}", status=500)
