...
class LockNotificationFacePlaceholder(QWidget):
    """Muted face mark shown until the RTSP camera sends its first frame."""
...
class SystemLockedNotification(QDialog):
    """Compact desktop lock notification with a read-only RTSP camera preview.
    ...
    an independent read-only subscriber to the local RTSP preview stream;
    it never controls camera capture, recognition, registration, or lock state.
    """
    ...
    def __init__(
        ...
        # line or red tint overlays are drawn over the live camera feed.
    ...
    # --------------------------------------------------------
    # Read-only RTSP camera feed
    # --------------------------------------------------------
    def _start_video_stream(self):
        if self._video_stream_running:
            return

        if not self.video_feed_url and self.frame_provider is None:
            self._set_live_status("Camera unavailable")
            return

        if cv2 is None and self.frame_provider is None:
            self._set_live_status("Camera unavailable")
            return
    ...
    def restart_video_stream(self):
        self._stop_video_stream()
        self._video_received_first_frame = False
        QTimer.singleShot(150, self._start_video_stream)

    def _video_worker(self):
        """Read preview frames without controlling camera lock state."""
        capture = None
        stream_failed = False

        try:
            if self.frame_provider is not None:
                self._video_worker_from_provider()
                return
            ...
            capture = cv2_mod.VideoCapture(self.video_feed_url)
            ...
            while self._video_stream_running and not self._video_stream_stop.is_set():
                ...
                last_emit_at = now
                frame_data = cast(Any, frame)
                self.live_frame_ready.emit(frame_data.copy())
    ...
    def _video_worker_from_provider(self):
        """Render frames already captured by the local recognition engine."""
        stream_failed = False
        last_emit_at = 0.0
        empty_reads = 0
        try:
            while self._video_stream_running and not self._video_stream_stop.is_set():
                ...
                try:
                    ...
                    frame = cast(Any, provider()) if provider is not None else None
                except Exception:
                    frame = None
                ...
                if frame is None:
                    empty_reads += 1
                    if empty_reads >= 12:
                        self.live_status_changed.emit("Connecting")
                    if empty_reads >= 150:
                        self.live_status_changed.emit("Reconnecting")
                    time.sleep(0.08)
                    continue
                ...
                try:
                    ...
                    self.live_frame_ready.emit(frame)
        except Exception:
            stream_failed = True
            try:
                self.live_status_changed.emit("Camera unavailable")
    ...
    def _render_live_frame(self, frame):
        ...
        try:
            ...
            if cv2 is not None:
                cv2_mod = cast(Any, cv2)
                rgb = cv2_mod.cvtColor(frame_data, cv2_mod.COLOR_BGR2RGB)
            else:
                rgb = frame_data[:, :, ::-1].copy()
    ...
    def build_ui(self):
        ...
        # Keep the live camera preview unobstructed.  The scanner line and red
...
class ConnectionSpinnerWidget(QWidget):
    """Small animated loading ring for camera connection handshake."""
...
class CameraConnectionDialog(QDialog):
    """Card-style camera connection popup.
    ...
    def __init__(
        ...
        self.setObjectName("CameraConnectionDialog")
        self.setWindowFlags(
            Qt.WindowType.Dialog
            | Qt.WindowType.FramelessWindowHint
            | Qt.WindowType.WindowStaysOnTopHint
    ...
    def force_close(self):
        """Close the camera connection dialog from the top-right X or action button.
    ...
    def build_ui(self):
        ...
        self.card = QFrame()
        self.card.setObjectName("CameraConnectionCard")
        ...
        self.visual_panel.setObjectName("CameraConnectionVisualLoading")
        ...
        self.icon_label.setObjectName("CameraConnectionLargeIcon")
        self.icon_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.icon_label.hide()

        self.visual_caption = QLabel("CONNECTING")
        self.visual_caption.setObjectName("CameraConnectionVisualCaption")
        ...
        self.content_panel.setObjectName("CameraConnectionContent")
        ...
        self.title.setObjectName("CameraConnectionTitle")
        self.title.setWordWrap(True)

        self.subtitle = QLabel("Securing encrypted handshake protocol")
        self.subtitle.setObjectName("CameraConnectionSubtitle")
        self.subtitle.setWordWrap(True)

        content.addWidget(self.title)
        content.addWidget(self.subtitle)

        self.node_label = self.info_block("LOCAL RTSP", "FACE RECOGNITION ENGINE")
        ...
        self.action_btn.setObjectName("CameraConnectionActionButton")
    ...
    def info_block(self, label_text: str, value_text: str):
        block = QFrame()
        block.setObjectName("CameraConnectionInfoBlock")
        layout = QVBoxLayout(block)
        layout.setContentsMargins(12, 0, 0, 0)
        layout.setSpacing(4)

        label = QLabel(label_text)
        label.setObjectName("CameraConnectionInfoLabel")
        value = QLabel(value_text)
        value.setObjectName("CameraConnectionInfoValue")
    ...
    def set_success(self, users: int = 0, recognition: bool = False, capture: bool = False):
        ...
        self.visual_panel.setObjectName("CameraConnectionVisualSuccess")
        self.visual_panel.style().unpolish(self.visual_panel)
        self.visual_panel.style().polish(self.visual_panel)
        ...
        self.subtitle.setText("Local RTSP stream is live.")
        self.node_label.hide()
        self.gateway_label.hide()
    ...
    def set_disconnected(self, message: str = "Local RTSP connection released."):
        ...
        self.icon_label.show()
        self.visual_caption.setText("RELEASED")

        self.title.setText("DISCONNECTED")
        self.subtitle.setText(message or "Local RTSP connection released.")
        self.node_label.hide()
        self.gateway_label.hide()
        self.status_label.hide()

        self.action_btn.setText("OK")
        self.action_btn.show()
        self.close_btn.setEnabled(True)
        self.close_btn.show()
        self.close_btn.raise_()
    def set_failed(self, error_text: str = ""):
        ...
        self.visual_panel.setObjectName("CameraConnectionVisualFailed")
        self.visual_panel.style().unpolish(self.visual_panel)
        self.visual_panel.style().polish(self.visual_panel)
        ...
        self.subtitle.setText("Please set/check camera hostname or IP in Settings, then connect again.")
        self.node_label.show()
        self.gateway_label.show()
        self.status_label.show()
        self.info_value_label(self.gateway_label).setText(self.host)
        self.info_value_label(self.status_label).setText("OFFLINE")
        self.info_value_label(self.node_label).setText("FACE RECOGNITION ENGINE")
