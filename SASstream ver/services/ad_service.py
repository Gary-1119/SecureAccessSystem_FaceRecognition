...
class ActiveDirectoryService:
    ...
    def validate_ntid_exists_with_reason(self, ntid: str) -> tuple[bool, str]:
        """Validate that an NTID exists in AD and return a user-facing reason."""
        safe_ntid = str(ntid or "").strip()
        if not safe_ntid:
        ...
        <soap12:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
          ...
          <soap12:Body>
            <IsUserExistsInAD xmlns="http://jpetewebapp/jtesw_ws/">
              <userName>{html.escape(safe_ntid)}</userName>
            </IsUserExistsInAD>
          </soap12:Body>
        </soap12:Envelope>"""
        ...
        try:
            self.debug(f"Register NTID AD validation for: {safe_ntid}")
            response = requests.post(
                self.soap_url,
                data=soap.encode("utf-8"),
                headers=headers,
            ...
            self.debug(f"Register NTID AD validation HTTP status: {response.status_code}")
            response.raise_for_status()
            if self.parse_ad_response(response.text):
                return True, "NTID exists in Active Directory."
            return False, "NTID does not exist in Active Directory."
        except requests.exceptions.Timeout:
            return False, "Active Directory SOAP request timed out."
        except requests.exceptions.ConnectionError:
            return False, "Active Directory SOAP service is unreachable."
        except Exception as exc:
            return False, f"Active Directory validation error: {type(exc).__name__}: {exc}"
