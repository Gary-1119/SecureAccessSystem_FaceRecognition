...
EMERGENCY_WATCHDOG_INTERVAL_SECONDS = 0.05
HOTKEY_REREGISTER_SECONDS = 2.0
...
class RuntimeLockService:
    ...
    def __init__(self):
        ...
        self._hotkey_last_status_print = 0
    ...
    def emergency_unlock(self, dashboard):
        """Unlock desktop input through the emergency hotkey without SAS login.

        Ctrl + Shift + Alt has the same workstation-unlock effect as a valid
        face recognition result: keyboard/mouse blocking is released and the
        inactivity countdown is reset. It does not create an Admin session,
        restore SAS, open a restricted page, or bypass the normal login required
        for Settings and Face Recognition.
        """
        try:
            workstation_user = (
            ...
            workstation_user = str(workstation_user).strip().upper() or "UNKNOWN"

            dashboard.append_system_log(
                "Emergency hotkey unlock activated. Desktop input released; SAS Admin login is still required for restricted pages."
            )
            if hasattr(dashboard, "write_sas_log"):
                dashboard.write_sas_log(
                    "UNLOCKED | USER=NOT_AUTHENTICATED | METHOD=EMERGENCY_HOTKEY "
                    f"| WORKSTATION_USER={workstation_user} | ACCESS=DESKTOP_ONLY | CONFIDENCE=N/A"
                )
            ...
            # The workstation is unlocked, but no SAS identity is authenticated.
            # Clear any emergency/previous session state so Settings and Face
            # Recognition always demand the standard Admin login flow.
            dashboard.is_logged_in = True
            dashboard.current_user = None
            dashboard.current_user_role = "Guest"
            dashboard.is_admin = False
            dashboard.pending_restricted_tab = None
            ...
            # The header returns to the normal Login state. The dashboard stays
            # minimized/backgrounded; the hotkey must not open any SAS page.
            ...
            # Real face unlock is now delivered by the local unlock channel.
    ...
    def start_emergency_hotkey_watchdog(self, dashboard):
        ...
        def watchdog_loop():
            while self._emergency_watchdog_running:
                ...
                time.sleep(EMERGENCY_WATCHDOG_INTERVAL_SECONDS)
    ...
    def poll_emergency_hotkey_state(self, dashboard):
        ...
        ctrl = pressed(0x11) or pressed(0xA2) or pressed(0xA3)
        shift = pressed(0x10) or pressed(0xA0) or pressed(0xA1)
        alt = pressed(0x12) or pressed(0xA4) or pressed(0xA5)

        if ctrl and shift and alt:
            now = time.time()

            # Debounce so one key hold does not trigger repeatedly.
            if now - self._emergency_last_fire < 0.7:
                return

            self._emergency_last_fire = now
            dashboard.qt_after(0, lambda: self.emergency_unlock(dashboard))

    def refresh_emergency_hotkey_if_needed(self, dashboard, force: bool = False):
        ...
        if not force and not getattr(dashboard, "is_locked", False):
            return

        now = time.time()
        if not force and now - self._hotkey_last_registered < HOTKEY_REREGISTER_SECONDS:
            return

        self._register_keyboard_hotkey(dashboard)

    def force_refresh_emergency_hotkey(self, dashboard):
        self.start_emergency_hotkey_watchdog(dashboard)
        self.refresh_emergency_hotkey_if_needed(dashboard, force=True)

    def recover_after_windows_session_change(self, dashboard):
        """Re-arm hooks after Windows lock/unlock or secure desktop transitions."""
        if getattr(dashboard, "enable_hotkey", True):
            self.force_refresh_emergency_hotkey(dashboard)

        if not getattr(dashboard, "is_locked", False):
            return

        if getattr(dashboard, "disable_mouse_when_locked", False):
            if not self._mouse_lock_running:
                self.disable_mouse(dashboard)
            elif self._mouse_hook_handle is None:
                self._install_mouse_click_block_hook(dashboard)

    def _register_keyboard_hotkey(self, dashboard):
        if keyboard is None:
            return
    ...
    def register_emergency_hotkey(self, dashboard, force: bool = False):
        # Always start backup watchdog, even if keyboard module is unavailable.
        self.start_emergency_hotkey_watchdog(dashboard)

        if keyboard is None:
            if not getattr(self, "_hotkey_missing_warned", False):
                self._hotkey_missing_warned = True
                dashboard.append_system_log("Emergency hotkey fallback active: keyboard module is not installed.")
            return

        now = time.time()
        if self._hotkey_registered and not force and now - self._hotkey_last_registered < HOTKEY_REREGISTER_SECONDS:
            return

        try:
            was_registered = self._hotkey_registered
            self._register_keyboard_hotkey(dashboard)
            if not was_registered or force or now - float(getattr(self, "_hotkey_last_status_print", 0) or 0) >= 30.0:
                self._hotkey_last_status_print = time.time()
    ...
    def disable_keyboard(self, dashboard):
        ...
        try:
            ...
            if getattr(dashboard, "enable_hotkey", True) and not getattr(self, "_shutting_down", False):
                self.register_emergency_hotkey(dashboard, force=True)
    ...
    def enable_keyboard(self, dashboard):
        ...
        try:
            if not self._keyboard_disabled:
                if getattr(dashboard, "enable_hotkey", True):
                    self.register_emergency_hotkey(dashboard)
                return
