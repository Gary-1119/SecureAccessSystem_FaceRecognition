...
from PySide6.QtWidgets import QDialog, QFrame, QGraphicsDropShadowEffect, QGraphicsOpacityEffect, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QScrollArea, QSizePolicy, QStackedWidget, QVBoxLayout, QWidget
...
class DashboardViewMixin:
    def build_dashboard_page(self: Any):
        outer = QWidget()
    ...
    def build_time_section(self: Any):
        section = QWidget()
    ...
    def build_unified_dashboard_card(self: Any):
    ...
    def build_bento_cards(self: Any):
        grid = QGridLayout()
    ...
    def card_padding(self: Any):
        return (34, 30) if not self.compact_mode else (22, 18)

    def status_card(self: Any):
    ...
    def toggle_lock_state(self: Any):
    ...
    def apply_lock_state(self: Any):
        """Update the dashboard card and keep the lock popup responsive.

        First-run keyboard/mouse hooks can take a moment to install. Show the
        no-focus lock notification before applying those hooks so users get
        immediate visual feedback that SAS is locked.
        """
        ...
        if self.is_locked:
            ...
            QTimer.singleShot(20, lambda: self.runtime_lock_service.apply_lock_controls(self))
        else:
            self.hide_system_locked_notification()
            self.runtime_lock_service.apply_lock_controls(self)

    def countdown_card(self: Any):
        card = GlassCard()
        ...
        # Runtime face unlock now arrives from local unlock events.
        card.mousePressEvent = lambda event: self.set_face_detected(not self.face_detected)

        return card

    def build_timer_circle_front(self: Any, circle_size: int):
        front = QWidget()
    ...
    def build_face_circle_back(self: Any, circle_size: int):
        back = QFrame()
    ...
    def set_face_detected(self: Any, detected: bool):
        """Update face state.
    ...
    def update_face_detected_label(self: Any, detected: bool):
    ...
    def grab_circle_face(self: Any, widget: QWidget) -> QPixmap:
    ...
    def flip_countdown_circle(self: Any, target_index: int, detected: bool):
    ...
    def users_card(self: Any):
        ...
        self.dashboard_users_status = QLabel("STATUS     Connect Camera")
    ...
    def clear_layout_widgets(self: Any, layout):
        if layout is None:
            return
        while layout.count():
    ...
    def clear_all_authorized_users_ui(self: Any):
        """Clear cached authorised users when camera is offline/disconnected."""
    ...
    def render_dashboard_users(self: Any, users):
        ...
        if not users:
            ...
            if isinstance(status_label, QLabel):
                status_label.setText("STATUS     Connect Camera")

            hint = QLabel("Please connect camera hostname/IP to fetch authorised users.")
    ...
    def open_authorized_users_popup(self: Any):
    ...
    def show_authorized_users_dialog(self: Any, users, connected=True):
        ...
        def popup_row(user, display_no: int):
            ...
            detail = QLabel(f"{photos} face sample{'s' if photos != 1 else ''} stored")
        ...
        def render_list():
            ...
            if not connected:
                ...
                msg = QLabel("Please connect camera hostname/IP first to fetch authorised users.")
    ...
    def user_row(self: Any, user_id, detail, active=True):
        row = QFrame()
