...
internal static class Program
{
    ...
    [
        ...
        ("PiJson reads string lists from object arrays", TestPiJsonStringListObjectArrays),
        ...
        ("PiConnectionController does not treat disconnected as connected", TestDisconnectedStateIsNotConnected),
        ("PiConnectionController stops old session before reconnect", TestPiConnectionStopsOldSessionBeforeConnect),
        ("PiConnectionController reports wrong hostname as disconnected", TestPiConnectionWrongHostnameFails),
        ("LogsController downloads Pi logs from transfer service", TestLogsControllerDownloadsPiLogs),
    ...
    private static Task TestAppSettingsHostNormalization()
    {
        ...
        AssertEqual("JEFACEDESKTOP", settings.PiHost);
    ...
    private static Task TestPiJsonStringListObjectArrays()
    {
        ...
        var values = PiJson.ReadStringList(data, "pending");
    ...
    {
        var transfer = new FakePiTransferService();
    ...
    private static Task TestFaceDataPathHelpers()
    {
        ...
        AssertEqual("/mnt/pcshare/backup.zip", FaceDataController.GetMountedServerZipPath(@"\\server\share\folder\backup.zip"));
    ...
    private static Task TestDisconnectedStateIsNotConnected()
    {
        ...
        AssertFalse(PiConnectionController.IsEffectivelyConnected(status), "Disconnected must not match the connected state.");
        return Task.CompletedTask;
    }

    private static async Task TestPiConnectionStopsOldSessionBeforeConnect()
    {
        ...
        var controller = new PiConnectionController(recognition, CreateLogService());
        ...
        AssertEqual("JEFACE", recognition.ConfiguredSettings.PiHost);
    }

    private static async Task TestPiConnectionWrongHostnameFails()
    {
        ...
        AssertEqual("WRONGHOST", recognition.ConfiguredSettings.PiHost);
    }

    private static async Task TestLogsControllerDownloadsPiLogs()
    {
        var transfer = new FakePiTransferService();
        transfer.PiLogs = ["line 1", "line 2"];
        var controller = new LogsController(CreateLogService(), transfer);
        var folder = Path.Combine(Path.GetTempPath(), "SAS_AUTOMATION_TESTS", Guid.NewGuid().ToString("N"));

        var result = await controller.DownloadPiLogAsync(new AppSettings { CameraHost = "JEFACE" }, folder);

        AssertTrue(File.Exists(result.Path), "Pi log file should be written.");
        AssertSequence(["line 1", "line 2"], await File.ReadAllLinesAsync(result.Path));
