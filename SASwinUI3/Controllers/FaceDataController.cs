...
{
    private readonly IPiTransferService _transferService;
    private readonly AppLogService _log;

    public FaceDataController(IPiTransferService transferService, AppLogService log)
    {
    ...
    public async Task<int> ImportAsync(FaceDataImportRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var added = request.ImportLocal
            ...
            : await _transferService.ImportFaceDataFromServerAsync(request.PiServerZipPath, cancellationToken);
    ...
    {
        return _transferService.ListSftpTargetsAsync(cancellationToken);
    }

    public Task AddSftpTargetAsync(string hostname, CancellationToken cancellationToken = default)
    {
        return _transferService.AddSftpTargetAsync(hostname, requireReachable: false, cancellationToken);
    }

    public async Task RemoveSftpTargetsAsync(IEnumerable<string> targets, CancellationToken cancellationToken = default)
    {
        foreach (var target in targets)
        {
            await _transferService.RemoveSftpTargetAsync(target, cancellationToken);
        }
    }

    public async Task<string> StartSftpTransferAsync(IReadOnlyList<string> targets, CancellationToken cancellationToken = default)
    {
        var batchId = await _transferService.StartMultiSftpTransferAsync(targets, cancellationToken);
        _log.Write($"SFTP transfer started: {batchId}");
        _log.WriteAudit("SFTP TRANSFER STARTED", $"BatchId={batchId}; Targets={string.Join(", ", targets)}");
        return batchId;
    }

    public Task<JsonObject> GetSftpTransferAsync(string batchId, CancellationToken cancellationToken = default)
    {
        return _transferService.GetMultiSftpTransferAsync(batchId, cancellationToken);
    }

    public Task<JsonObject> CancelSftpTransferAsync(string batchId, CancellationToken cancellationToken = default)
    {
        return _transferService.CancelMultiSftpTransferAsync(batchId, cancellationToken);
    }

    public Task<IReadOnlyList<string>> ListReceivedAsync(CancellationToken cancellationToken = default)
    {
        return _transferService.ListReceivedAsync(cancellationToken);
    }

    public Task<PiStorageInfo> GetStorageInfoAsync(CancellationToken cancellationToken = default)
    {
        return _transferService.GetStorageInfoAsync(cancellationToken);
    }

    public Task<int> AcceptReceivedAsync(string filename, CancellationToken cancellationToken = default)
    {
        return _transferService.AcceptReceivedAsync(filename, cancellationToken);
    }

    public Task RejectReceivedAsync(string filename, CancellationToken cancellationToken = default)
    {
        return _transferService.RejectReceivedAsync(filename, cancellationToken);
    }
    ...
    public static string GetMountedServerZipPath(string importPath)
    {
        var value = NormalizeServerPath(importPath);
        var filename = value.Split('/', StringSplitOptions.RemoveEmptyEntries).LastOrDefault() ?? value;
        return "/mnt/pcshare/" + filename;
    }
...
public sealed record FaceDataImportRequest(bool ImportLocal, string ImportZipPath, string PiServerZipPath, AppSettings? ServerSettings);
