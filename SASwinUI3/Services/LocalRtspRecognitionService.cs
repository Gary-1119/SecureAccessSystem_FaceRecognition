...
public sealed class LocalRtspRecognitionService : IRecognitionService, IJabilEyeTransferService
{
    ...
    private void TryRecognize(byte[] jpeg)
    {
        ...
        SetFaceMatch(best.User, best.Confidence, embedding.FaceBox);
        _lastUnlockAt = now;
    ...
    private void SetFaceMatch(FaceUserRecord user, double confidence, RecognitionFaceBox? faceBox)
    {
        PublishStatus(_statusStore.SetFaceMatch(user, confidence, faceBox));
    }
