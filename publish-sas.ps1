...
Write-Host "User data path preserved: $env:LOCALAPPDATA\SASstream"
...
$dataRoot = Join-Path $env:LOCALAPPDATA "SASstream"
Write-Host ""
...
Write-Host $dataRoot
Write-Host ""
$answer = Read-Host "Keep user data? Press Enter or type KEEP to keep saved hostname, admins, settings, logs, and face data. Type DELETE only to remove all user data"
if ($answer -eq "DELETE") {
    Remove-Item -LiteralPath $dataRoot -Recurse -Force -ErrorAction SilentlyContinue
    Write-Host "User data deleted."
} else {
    Write-Host "User data kept for future SAS updates."
}
...
4. Existing user data in `%LocalAppData%\SASstream` is preserved during updates.

To check whether SAS is registered in HKCU startup:

```powershell
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" | Select-Object "Secure Access System"
```

Expected value:

```text
"%LocalAppData%\Programs\SAS\App\SAS.exe"
```
